//! Opaque-API unit tests for `zensim-gpu`.
...
fn flat_weights() -> [f64; TOTAL_FEATURES] {
    // Flat-weight vector for the bit-identity test: sums every
    // feature equally. `score_from_features` then becomes a function
    // of the features alone, which is exactly what we want to compare
    // opaque vs typed without depending on a trained profile that
...
fn opaque_srgb_u8_matches_typed() {
    ...
    let mut typed = Zensim::<BackendT>::new(client, w, h).expect("typed new");
    let typed_features = typed
        .compute_features(&ref_buf, &dis_buf)
        .expect("typed compute_features");
    ...
    let rel = (opaque_score.value - typed_value).abs() / typed_value.abs().max(1e-12);
    assert!(
        rel < 1e-5,
    ...
    assert_eq!(opaque_score.metric_name, "zensim");
...
fn opaque_pixels_handles_stride() {
    ...
    let mut opaque = ZensimOpaque::new(
        ...
        w,
        h,
        ZensimParams::new().with_weights(flat_weights()),
    )
    .expect("opaque new");
