//! The typed `Dssim<R>` public API pads sub-8px itself (not just the
//! `DssimOpaque` wrapper), so no public interface returns a degenerate
...
fn typed_sub8_score_matches_manual_pad_to_8() {
    ...
    let mut z = Dssim::<Bt>::new(Bt::client(&Default::default()), w, h).expect("sub-8 pads");
