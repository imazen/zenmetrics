//! Sub-8px reflect-pad: DSSIM's typed 5-scale pipeline rejects `< 8×8`;
//! `DssimOpaque` reflect(mirror)-pads sub-8px inputs up to that floor so
//! it returns a finite score down to 1×1 instead of `InvalidImageSize`,
//! and a constant colour difference scores identically for all sizes at
//! or below the floor (they map to the same padded image).

#![cfg(all(feature = "cubecl-types", any(feature = "cuda", feature = "wgpu")))]

use dssim_gpu::{Backend, DssimOpaque, DssimParams};
...
fn score(w: u32, h: u32, r: &[u8], d: &[u8]) -> f64 {
    DssimOpaque::new(BACKEND_E, w, h, DssimParams::default())
        .expect("opaque new (sub-8 reflect-pad must not error on ≥1px)")
        .compute_srgb_u8(r, d)
        .expect("dssim compute")
        .value
}
...
fn scores_every_size_down_to_1px() {
    for n in [1u32, 2, 3, 4, 7, 8, 16] {
        ...
        let mut z = DssimOpaque::new(BACKEND_E, n, n, DssimParams::default()).expect("new");
        assert_eq!(z.dims(), (n, n), "dims() must report logical size at {n}px");
        let s = z.compute_srgb_u8(&r, &d).expect("score").value;
        assert!(s.is_finite(), "dssim score at {n}px must be finite, got {s}");
    }
}
...
fn solid_color_diff_invariant_at_or_below_floor() {
    ...
    eprintln!("dssim solid-diff ≤floor {sizes:?} = {scores:?} (spread {:.6})", hi - lo);
    assert!(
        hi - lo < 1e-4,
        "dssim solid-colour difference must be invariant for sizes ≤ 8px floor; spread {:.6}",
        hi - lo
    );
}
...
fn nonsquare_sub8_scores_finite() {
    for (w, h) in [(2u32, 24u32), (24, 2), (1, 30), (30, 1), (5, 3)] {
        ...
        assert!(s.is_finite(), "dssim {w}x{h} score must be finite, got {s}");
    }
}
