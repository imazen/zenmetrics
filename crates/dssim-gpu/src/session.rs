...
use crate::Result;
use crate::opaque::{Backend, DssimOpaque, DssimParams};

// Pool cleanup / usage are metric-agnostic — re-export the shared impls so
// `dssim_gpu::session::{cleanup_stream, stream_reserved_bytes}` keep resolving.
#[doc(hidden)]
pub use zenmetrics_gpu_core::{cleanup_stream, stream_reserved_bytes};

/// Build an opaque dssim scorer bound to the private cubecl stream
...
pub fn new_opaque_on_stream(
    ...
    match backend {
        #[cfg(feature = "cuda")]
        Backend::Cuda => DssimOpaque::build_from_client::<cubecl::cuda::CudaRuntime>(
        ...
        #[cfg(feature = "wgpu")]
        Backend::Wgpu => DssimOpaque::build_from_client::<cubecl::wgpu::WgpuRuntime>(
        ...
        #[cfg(feature = "cpu")]
        Backend::Cpu => DssimOpaque::build_from_client::<cubecl::cpu::CpuRuntime>(
