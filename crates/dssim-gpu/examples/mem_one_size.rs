//! GPU memory measurement driver for dssim-gpu.
...
fn main() {
    ...
    let score: f64 = match mode.as_str() {
        "full" => {
            let mut d_ssim = Dssim::<Backend>::new(client, w, h).expect("Dssim::new");
            d_ssim.set_reference(&r).expect("set_reference");
            let res = d_ssim.compute_with_reference(&d).expect("compute_with_reference");
            res.score
        }
        "strip" => {
            let mut d_ssim = Dssim::<Backend>::new_strip(client, w, h, 256).expect("Dssim::new_strip");
            let res = d_ssim.compute(&r, &d).expect("compute strip");
            res.score
        }
