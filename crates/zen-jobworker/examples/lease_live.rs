...
//!     cargo run -p zen-jobworker --example lease_live
//!
//! Not a #[test] — it needs creds + network, so the *caller* decides when to run it.

use zen_job_core::{sha256, JobId, JobKind};
use zen_jobworker::claim_or_steal_r2;

fn main() {
    let ep = std::env::var("ZEN_R2_ENDPOINT").expect("set ZEN_R2_ENDPOINT");
    let bucket = std::env::var("ZEN_LEASE_BUCKET").unwrap_or_else(|_| "zen-tuning-ephemeral".into());
    let prefix = std::env::var("ZEN_LEASE_PREFIX").expect("set ZEN_LEASE_PREFIX");
    let kind = JobKind::Metric { metric: "cvvdp".into() };
