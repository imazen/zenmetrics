...
| Per-band pooling   | `kernels/pool`         | GPU `pool_band_3ch_kernel` (fused 3-channel, atomic f32 partials, one launch per pyramid band) consumed by `compute_dkl_jod`. Single-channel `pool_band_kernel` retained as a test-only entry point. | 3 host fixtures + GPU vs lp_norm_mean (single-channel form covered by `pool_band_kernel_matches_host_lp_norm_mean`)     |
