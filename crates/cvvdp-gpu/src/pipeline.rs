...
impl<R: Runtime> Cvvdp<R> {
    ...
    pub fn new(
        ...
        let src_ref = client.create_from_slice(&vec![0u8; n0 * 3]);
        let src_dis = client.create_from_slice(&vec![0u8; n0 * 3]);
        let srgb_lut = client.create_from_slice(f32::as_bytes(&vec![0.0f32; 256]));
