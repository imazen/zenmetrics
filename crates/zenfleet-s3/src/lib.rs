//! `zenfleet-s3` — the shared S3-compatible storage helper for the
//! `zenfleet-cloud` worker trait layer.
//!
...
//!    `zenfleet-vastai`'s `worker::r2::R2Client`. The only change is
//!    the constructor, which now takes plain fields so any provider can
//!    build one without a vast.ai `WorkerArgs`.
//! 2. [`S3BlobStorage`] — the [`zenfleet_cloud::BlobStorage`] impl over
//!    [`S3Client`], relocated from `zenfleet-vastai`'s
//!    `cloud::R2BlobStorage`.
//!
//! `zenfleet-vastai` re-exports both under their historical names
