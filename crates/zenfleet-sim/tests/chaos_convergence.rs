...
fn status_of<'a>(r: &'a HashMap<zenfleet_core::JobId, JobStatus>, j: &JobSpec) -> &'a JobStatus {
    r.get(&j.desired.job_id())
        .expect("job has a terminal status")
}
...
fn deterministic_failure_poisons_without_endless_retry() {
    ...
    assert!(
        r.passes <= 3,
        "poisoned promptly, not retried forever (passes={})",
        r.passes
    );
}
...
fn transient_failure_poisons_at_the_attempt_cap() {
    ...
    assert_eq!(
        r.poison, 1,
        "a job that never stops timing out is poisoned at the cap"
    );
    assert_eq!(status_of(&r.final_status, &jobs[0]), &JobStatus::Poison);
}
...
fn converges_despite_a_flaky_store() {
    ...
    assert!(
        r.converged,
        "converged despite substrate chaos (passes={})",
        r.passes
    );
    ...
    assert!(
        r.claim_errors > 0,
        "the flaky store induced retries that still converged"
    );
}
