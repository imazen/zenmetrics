...
//! GPU SSIMULACRA2 score via the `ssim2-gpu` crate.
//!
//! Picks the CubeCL runtime requested by `--gpu-runtime`. `auto` walks the
//! compiled-in list (CUDA → wgpu → HIP → CPU) and returns the first one
//! that initialises and produces a finite score.
...
pub fn score(
    ...
    Err(format!(
        "ssim2-gpu: no runtime succeeded; last error: {}",
...
fn run<R: Runtime>(
    ...
    let mut s = ssim2_gpu::Ssim2::<R>::new(client, reference.width, reference.height)
        .map_err(|e| format!("Ssim2::new: {e}"))?;
    let result = s
        .compute(&reference.pixels, &distorted.pixels)
        .map_err(|e| format!("Ssim2::compute: {e}"))?;
    let score = result.score;
    if !score.is_finite() {
        return Err(format!("ssim2-gpu produced non-finite score: {score}").into());
    }
    Ok(score)
}
