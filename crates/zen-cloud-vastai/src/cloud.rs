//! vast.ai implementations of the [`zen_cloud_core`] trait surface.
//!
...
//! `zen_cloud_core::run_worker` loop (and the Phase-B `local` backend)
//! can drive vast.ai with no provider knowledge.
//!
//! Phase A does NOT route the production worker through these — the
//! `vastai-fleet`/`zen-sweep-worker` deployed worker stays on the
