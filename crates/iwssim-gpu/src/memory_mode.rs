...
/// the result. Mirrors `zenfleet_vastai::worker::adapt::nvidia_smi_total_memory_mb`
...
//
/// iwssim-gpu has **no committed timing benchmark**, so this is a
/// deliberately conservative flat rate (not a measured value). 100 ns/px
/// sits at the high end of the measured multiscale-metric costs
/// (cvvdp medium 28 ns/px, small 91 ns/px). IW-SSIM's information-weighted
/// per-band pooling adds work on top of a plain SSIM pyramid, so a value at
/// the high end errs toward *over*-provisioning — safe for capacity
/// planning, never under-sizes a box. Replace with an anchored piecewise
/// curve once a `time_size_sweep` bench lands for this crate.
const IWSSIM_NS_PER_PX_PLACEHOLDER: f64 = 100.0;
/// Conservative fixed per-call overhead (ms). NOT CALIBRATED — see
/// [`IWSSIM_NS_PER_PX_PLACEHOLDER`].
const IWSSIM_FIXED_MS_PLACEHOLDER: f64 = 3.0;

/// Estimated single-GPU score wall time (ms) for a `width × height` IW-SSIM
/// pair. **PLACEHOLDER, NOT CALIBRATED** — a conservative `fixed +
/// per-pixel` upper-leaning estimate (see [`IWSSIM_NS_PER_PX_PLACEHOLDER`]);
/// intended only so the fleet planner has a non-zero iwssim cost until a
/// real size-sweep is run. Pure size-math, no GPU. Returns `0.0` for a
/// degenerate image.
#[must_use]
pub fn estimate_score_time_ms(width: u32, height: u32) -> f32 {
    ...
    (IWSSIM_FIXED_MS_PLACEHOLDER + IWSSIM_NS_PER_PX_PLACEHOLDER * pixels / 1.0e6) as f32
}
...
///
/// **Note**: iwssim's `time_ms` is a non-calibrated placeholder — see
