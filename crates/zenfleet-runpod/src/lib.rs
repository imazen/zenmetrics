//! `zenfleet-runpod` — the RunPod provider for the `zenfleet-cloud`
...
//!   is reused verbatim from [`zenfleet_vastai`].
...
//!
//! - [`queue::RunpodChunkQueue`] — a [`zenfleet_cloud::JobQueue`] over an
//!   R2 `chunks.jsonl` manifest with the shared R2-ETag atomic claim
//!   (reuses [`zenfleet_vastai::worker::claim::try_claim`] — no third
//!   claim impl). `ack_chunk` is a no-op (the claim + sidecar are the
//!   durable state).
//! - [`host::RunpodEnvCredentials`] — a [`zenfleet_cloud::CredentialSource`]
...
//! - [`host::RunpodWorkerHost`] — a [`zenfleet_cloud::WorkerHost`] over
//!   `RUNPOD_POD_ID` / `RUNPOD_GPU_COUNT` / `nvidia-smi`.
//! - [`storage::RunpodBlobStorage`] — re-exported from the shared
//!   `zenfleet-s3` crate (RunPod has no native object store; workers
