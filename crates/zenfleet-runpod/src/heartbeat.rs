...
//! vast.ai backend does — reusing the shared [`zenfleet_s3`]
//! `BlobStorage` (no second R2 writer). Operators that rely solely on
//! the RunPod dashboard can use [`NoopHeartbeat`] instead.

use zenfleet_cloud::{ArtifactKey, BlobStorage, CloudError, Heartbeat, WorkerId, WorkerStatus};
use zenfleet_s3::S3BlobStorage;
...
mod tests {
    ...
    fn r2_heartbeat_key_normalises_prefix() {
        let client = zenfleet_s3::S3Client::new("s5cmd", "https://e.example.com", "r2");
