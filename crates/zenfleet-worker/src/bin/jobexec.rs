...
//! no stub. Wire it as `zenfleet-worker --exec <path>/zenfleet-exec`. (Light path: JPEG/PNG via `image`;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let out = match &job.kind {
        ...
        other => {
            return Err(format!("zenfleet-exec handles Encode/Metric jobs (got {other:?})").into());
        }
    };
