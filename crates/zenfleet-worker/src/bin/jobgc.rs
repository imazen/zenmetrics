#![forbid(unsafe_code)]
//! `zenfleet-gc` — safe garbage collection over the blob index + ledger (goal G).
//!
...
//!
//!   zenfleet-gc --blob-index s3://b/jobsys/dashboard-seed/blob_index.parquet \
...
use clap::Parser;
use zenfleet_core::{JobStatus, LedgerView, Sha256Hex};
use zenfleet_worker::{GcExecCfg, gc_execute};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for l in &c.ledger {
        for r in zenfleet_ledger::read_ledger_uri(l, ep)? {
            view.apply(r);
        }
    }
    ...
    let index = zenfleet_ledger::read_blob_index_uri(&c.blob_index, ep)?;
    ...
    eprintln!(
        "zenfleet-gc: {} ({} index rows, {} referenced)",
    ...
    if !report.refused.is_empty() {
        eprintln!(
            "zenfleet-gc: REFUSED {} unreferenced irreplaceable blob(s) — surfaced, not deleted (pin or archive).",
            report.refused.len()
        );
    }
    Ok(())
