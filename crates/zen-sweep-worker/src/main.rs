...
//!   generic [`zen_cloud_core::run_worker`] loop with the RunPod Pods
...
mod runpod {
    ...
    pub fn run(args: WorkerArgs) -> Result<()> {
        ...
        let r2 = zen_cloud_vastai::worker::r2::new_from_args(&args)
        ...
        #[cfg(not(feature = "_runpod-sweep"))]
        {
            ...
            Err(CloudError::Compute(
                "runpod glue build has no encode+score compute; \
                 rebuild zen-sweep-worker with --features runpod-sweep"
                    .into(),
            ))
        }
    }
