# zen-cloud-runpod — RunPod provider
...
`zen-cloud-vastai` is GPU-free — it pulls only the claim + R2 plumbing,
not the codec tree.)
...
| `BlobStorage` | `storage::RunpodBlobStorage` | Re-export of the shared `zen-cloud-s3` impl. RunPod has no native object store; workers BYO R2/S3. **No second S3 client** (spec §1.10). |
...
1. `zen-sweep-worker` (built `--features runpod-sweep` — glue + the
