//! Paired interleaved A/B end-to-end bench for the W44 SIMD masking
//! chunk. Measures `Cvvdp::score` wall-time before vs after the
//! `mult_mutual_band_into` SIMD rewire.
//!
//! Internally we do:
//! - 3 warmup calls
...
//!
//! That stack defends against the high single-iter variance we
//! observe at 1024² (±15 ms standard deviation on this machine
//! under typical desktop load).
...
fn main() -> std::io::Result<()> {
    ...
    let path = output_path.unwrap_or_else(|| "/tmp/time_masking_paired_ab.tsv".to_string());
