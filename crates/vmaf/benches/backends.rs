...
use vmaf::{
    ModelVariant, VmafFeatures, VmafModel, Yuv420Frame, adm3_v1_from_luma, cambi_v1_from_luma,
    motion3_from_luma, score_v1_420, speed_v1_chroma_420,
};
use vmaf_head_sys::*;
...
fn score_c(reference: &[Frame], distorted: &[Frame], cpumask: u64) -> Vec<f64> {
    ...
    let name = CString::new("v1_bench").unwrap();
    let version = CString::new(ModelVariant::Phone.built_in_name()).unwrap();
