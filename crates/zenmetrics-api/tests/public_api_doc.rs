...
//! (`docs/public-api/<crate>.txt`, one per library crate) on every
...
/// One library crate: (package name, sections).
type CrateCfg = (&'static str, &'static [SectionCfg]);

/// API-relevant library crates in this workspace (the umbrella API crate,
/// the two CPU metric crates, and the six GPU metric crates). Fleet/jobdash
/// infra, corpus/test-only crates, codegen tools, and pure-bin crates are
    ...
    for (package, configs) in CRATES {
        ...
        doc.push_str(&format!(
            "# {package} public API surface\n\
             # Generated by crates/zenmetrics-api/tests/public_api_doc.rs via `cargo public-api --simplified`\n\
