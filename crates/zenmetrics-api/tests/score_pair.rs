...
fn score_pair_matches_manual_new_compute() {
    ...
    let r: Vec<u8> = (0..n).map(|i| ((i * 2654435761usize) >> 13) as u8).collect();
    let d: Vec<u8> = r
        .iter()
        .enumerate()
    ...
    let one_shot =
        score_pair(MetricKind::Cvvdp, Backend::Cuda, w, h, &r, &d).expect("score_pair");
    assert!(one_shot.value.is_finite(), "score_pair value must be finite");
