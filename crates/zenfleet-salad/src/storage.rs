//! SaladCloud `BlobStorage` — the shared S3-compatible store, BYO.
//!
//! SaladCloud has no native object store, so a Salad worker brings its
//! own R2/S3 bucket — exactly the storage the vast.ai backend uses. Per
//! spec §1.9 item 4 we do NOT write a second S3 client: this module is a
//! thin constructor around the shared [`zenfleet_s3::S3BlobStorage`]
//! (re-exported here as [`SaladBlobStorage`]) wiring it from the BYO
//! credentials the launcher set on the container group.

use zenfleet_cloud::CloudError;
use zenfleet_s3::{S3BlobStorage, S3Client};

/// `BlobStorage` for the Salad backend — the shared S3-compatible impl.
pub type SaladBlobStorage = S3BlobStorage;

/// Build a [`SaladBlobStorage`] from explicit endpoint/profile/binary.
///
/// `endpoint` is the BYO S3-compatible endpoint URL; `profile` is the
/// `~/.aws/credentials` profile (default `r2`); `s5cmd_bin` is the
/// s5cmd binary on PATH.
pub fn blob_storage(
...
/// Build a [`SaladBlobStorage`] from the resolved container-group
/// credentials map (the output of
/// [`crate::host::SaladEnvCredentials::resolve`]).
///
...
pub fn blob_storage_from_credentials(
...
) -> Result<SaladBlobStorage, CloudError> {
    let endpoint = if let Some(ep) = creds.get("R2_ENDPOINT").filter(|s| !s.is_empty()) {
        ...
        return Err(CloudError::Credentials(
            "neither R2_ENDPOINT nor R2_ACCOUNT_ID present in container-group env".into(),
        ));
    };
    let profile = creds
        .get("S5CMD_PROFILE")
