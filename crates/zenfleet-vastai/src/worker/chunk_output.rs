...
//! came from the same `zenmetrics sweep` invocation shape (same
...
//! `zenmetrics` processes that could in principle emit different
    ...
    if let Some(prefix) = encoded_r2_prefix {
        ...
        let uri_values: Vec<String> =
            if let Some(idx) = batch.schema().index_of("encoded_filename").ok() {
                ...
                // if zenmetrics ever switches the column type.
