//! `zenfleet-vastai` — the vast.ai provider for the `zenfleet-cloud`
//! trait layer (renamed from `zenfleet-vastai`, spec §1.7 Phase A).
//!
...
//!    `zenfleet-vastai`: the tolerant `vastai` JSON [`parse`]r, the async
//!    [`worker`] dispatch loop with adaptive concurrency + the R2
//!    token-race claim, and the `self-destroy` / `status` / `destroy`
//!    / `watch` operator CLI (in the `zenfleet-vastai` binary). This is
...
//!      R2.
//!
//! The new `zenfleet-sweep` binary depends on this crate and dispatches
//! `--backend vastai` to [`worker::cmd_worker`] — the SAME code path the
//! `zenfleet-vastai` binary runs, so output is byte-identical.
