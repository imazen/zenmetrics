//! End-to-end CLI tests for `zenfleet-vastai`. These shell out to the built
...
fn status_mixed_fleet_filters_by_label() {
    ...
    assert!(
        stdout.contains("instances:     5"),
        "expected 5 ssim2 instances; got: {stdout}"
    );
    // Burn rate: 0.0852 + 0.0911 + 0.1023 + 0.0 + 0.095 = 0.3736
    // (w4 is exited but still listed; dph 0.0).
    assert!(
        stdout.contains("0.374") || stdout.contains("0.373"),
        "expected ~0.374/hr; got: {stdout}"
    );
    ...
    assert!(
        stderr.contains("WARN parser"),
        "expected warning on null row; got stderr: {stderr}"
    );
}
...
fn destroy_dry_run_does_not_call_vastai() {
    ...
    assert!(
        out.status.success(),
        "destroy --dry-run should succeed; stderr={}",
        String::from_utf8_lossy(&out.stderr)
    );
    let stderr = String::from_utf8_lossy(&out.stderr);
    ...
    assert_eq!(
        n_dry, 5,
        "expected 5 dry-run destroys; got {n_dry}; stderr: {stderr}"
    );
}
...
fn status_json_format() {
    ...
    assert_eq!(
        v["label_prefix"].as_str(),
        Some("ssim2-backfill-2026-05-18")
    );
