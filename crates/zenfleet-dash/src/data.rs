...
use zenfleet_core::{BlobIndexEntry, LedgerRow, WorkerReport};
...
impl DashData {
    ...
    pub fn from_local(
        ...
        for p in ledger_paths {
            rows.extend(zenfleet_ledger::read_ledger(p)?);
        }
        let blobs = match blob_index {
            Some(p) => zenfleet_ledger::read_blob_index(p)?,
            None => Vec::new(),
        };
    ...
    pub fn from_sources(
        ...
        let mut rows = Vec::new();
        for uri in ledger {
            rows.extend(zenfleet_ledger::read_ledger_uri(uri, endpoint)?);
        }
        let blobs = match blob_index {
            Some(uri) => zenfleet_ledger::read_blob_index_uri(uri, endpoint)?,
            None => Vec::new(),
        };
        let workers = match workers_json {
            Some(uri) => {
                let bytes = zenfleet_ledger::read_bytes_uri(uri, endpoint)?;
                serde_json::from_slice(&bytes).map_err(|e| DashError::Json(e.to_string()))?
            }
            None => Vec::new(),
        };
...
impl From<zenfleet_ledger::LedgerError> for DashError {
    fn from(e: zenfleet_ledger::LedgerError) -> Self {
