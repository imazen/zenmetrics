...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let data = tokio::task::spawn_blocking(load)
        .await?
        .unwrap_or_else(|e| {
            eprintln!("zenfleet-dash: warning: initial load failed ({e}); serving empty views");
            DashData::default()
        });
    eprintln!(
        "zenfleet-dash: loaded {} rows, {} blobs, {} workers",
        data.rows.len(),
        data.blobs.len(),
        data.workers.len()
    );
    ...
    if fleet_token().is_none() {
        eprintln!(
            "zenfleet-dash: note: no HETZNER_API_TOKEN — kill controls record intent but won't actuate."
        );
    }
    ...
    eprintln!("zenfleet-dash listening on http://{addr} (web_dir={web_dir})");
...
async fn require_auth(req: Request, next: Next) -> Response {
    ...
    if ok {
        ...
        resp.headers_mut().insert(
            ...
            HeaderValue::from_static("Basic realm=\"zenfleet-dash\", charset=\"UTF-8\""),
        );
        resp
    }
}
...
fn load() -> Result<DashData, zenfleet_dash::DashError> {
...
async fn refresh_loop(state: Arc<RwLock<DashData>>) {
    ...
    loop {
        ...
        match tokio::task::spawn_blocking(load).await {
                ...
                if let Some(url) = webhook.as_deref() {
                    ...
                    for ev in detect(&prog, &cost, cap, poison_threshold) {
                        ...
                        if fired.insert(sig) {
                            ...
                            {
                                eprintln!("zenfleet-dash: webhook send failed: {e}");
                            }
                        }
                    }
                }
            ...
            Ok(Err(e)) => eprintln!("zenfleet-dash: ledger reload failed: {e}"),
            Err(e) => eprintln!("zenfleet-dash: reload task panicked: {e}"),
        }
    }
}
...
async fn api_control(
    ...
    let plan = match intent {
        ...
        ControlIntent::Pause { .. } => write_run_control(zenfleet_core::RunControl::PAUSED),
        ControlIntent::Drain { .. } => write_run_control(zenfleet_core::RunControl::DRAINING),
        ControlIntent::Resume { .. } => write_run_control(zenfleet_core::RunControl::RUNNING),
        other => serde_json::json!({ "intent": other, "status": "queued_for_fleet_actuation" }),
    };
    Json(plan)
}

/// Write a [`RunControl`](zenfleet_core::RunControl) object to R2 (goal C pause/drain actuation).
...
fn write_run_control(ctl: zenfleet_core::RunControl) -> serde_json::Value {
    ...
    match zenfleet_ledger::write_bytes_uri(&uri, &body, endpoint.as_deref()) {
        Ok(()) => {
...
async fn placeholder() -> Html<&'static str> {
    Html(
        "<!doctype html><meta charset=utf-8><title>zenfleet-dash</title>\
        <body style=\"font:14px system-ui;margin:3rem;max-width:40rem\">\
        <h1>zenfleet-dash</h1><p>API is live (<code>/api/progress</code>, <code>/api/cost</code>, \
        <code>/api/fleet</code>, …). The shadcn SPA build output was not found — \
        run <code>npm ci &amp;&amp; npm run build</code> in <code>crates/zenfleet-dash/web</code> \
