...
//! `zenfleet-local` dev mode. Demonstrates, with real I/O:
...
//!
//! Run: `cargo run -p zenfleet-dash --bin local_demo`
//! Then serve it: `ZEN_LEDGER=<printed path> cargo run -p zenfleet-dash` and open http://localhost:3000
...
use zenfleet_ledger::{compact_ledger, read_ledger, write_blob_index, write_ledger};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  ZEN_LEDGER={} ZEN_BLOB_INDEX={} ZEN_WORKERS_JSON={} PORT=3137 cargo run -p zenfleet-dash",
        lp.display(),
        bp.display(),
        wp.display()
    );
