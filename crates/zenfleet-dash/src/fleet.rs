...
//! `zenfleet-hetzner` so the Railway deploy workspace stays tiny (no fleet/salad/orchestrator stack);
//! the API surface mirrors `zenfleet-hetzner::api`.
//!
...
/// the rest conservatively. Mirrors `zenfleet-hetzner::api::urlencode`.
