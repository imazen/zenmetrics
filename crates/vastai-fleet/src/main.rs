//! `vastai-fleet` — robust replacement for the bash+python destroyer +
...
//!    `vastai-fleet watch` does the same thing in 50 LOC of Rust,
...
fn fetch_instances(
    ...
    for w in &report.warnings {
        eprintln!("[vastai-fleet] WARN parser: {w}");
    }
    Ok(report.instances)
}
...
fn cmd_destroy(
    ...
    eprintln!(
        "[vastai-fleet] destroying {} instances matching {label_prefix:?} (total ${:.3}/hr)",
        matched.len(),
        total_dph(&matched)
    );
...
fn cmd_watch(args: WatchArgs) -> anyhow::Result<()> {
    ...
    eprintln!(
        "[vastai-fleet watch] label_prefix={:?} target={} r2_prefix={} max_wall={}min poll={}s",
    ...
    loop {
        ...
        let (n_inst, dph) = match &inst_result {
            ...
            Err(e) => {
                eprintln!("[vastai-fleet watch] WARN fetch instances: {e}");
                (0, 0.0)
            }
        };

        eprintln!(
            "[vastai-fleet watch] t+{:.1}min  sidecars: {}/{}  fleet: {} (${:.3}/hr)",
