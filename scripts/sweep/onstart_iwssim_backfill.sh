#!/usr/bin/env bash
#
# onstart_iwssim_backfill.sh — vast.ai instance entry point for the
...
#   SWEEP_RUN_ID                 e.g. iwssim-backfill-2026-05-17
# Optional:
#   WORKER_ID                    defaults to $(hostname)-$$
#   PARALLEL                     concurrent chunk workers per box (default auto)
#   WORKDIR                      defaults to /workspace/iwssim-backfill
...
if [[ "$PARALLEL" == "0" ]]; then
    ...
    # IW-SSIM at 1024² needs ~50 MiB GPU per instance (vs cvvdp's
    # ~375 MiB). Cap at 1 instance per ~256 MiB free GPU mem to be
    # safe (the 4× factor includes intermediate pyramid buffers).
    gpu_cap=$cpu_cap
...
WORKDIR="${WORKDIR:-/workspace/iwssim-backfill}"
...
# libnvrtc12: same dance as cvvdp — cubecl-cuda needs runtime NVRTC.
...
log "pulling iwssim_backfill_chunk_worker.sh + chunks.jsonl from $SCRIPTS_R2_PREFIX"
...
process_chunk() {
    ...
    # Idempotent skip: if the iwssim sidecar already exists, we're done.
