#!/usr/bin/env bash
#
# ssim2_backfill_chunk_worker.sh — process one chunk of the
# ssim2-backfill fleet (V_24-mix-with-ssim2 retrain dependency).
#
# Adapted from iwssim_backfill_chunk_worker.sh — same chunk format,
# same R2 layout, just one scorer (ssim2-gpu) instead of iwssim.
# Output sidecar path: `out_sidecar_ssim2` (chunks.jsonl gets a new
# field; backfill scripts will populate it before upload).
#
# The worker:
...
#   5. Runs `zen-metrics score-pairs --metric ssim2-gpu` to produce the
#      ssim2 sidecar.
#   6. Uploads sidecar to out_sidecar_ssim2 from the chunk manifest.
#
# Audit note (EX-3, 2026-05-17): `score_ssim2` is NOT in the 372-feature
# input vector (zenanalyze emits image-property features `f0..f371`,
# not per-pair metric scores). Anti-pattern #6 ("Don't use the same
# metric as both target and feature") does NOT apply — ssim2 is
# target-only. The default `ssim2-gpu` IS the canonical SSIMULACRA2 v2
# (Sneyers/Wang); no separate "extended v2" toggle is required.
#
...
#       ssim2_backfill_chunk_worker.sh \
#           --zen-metrics-image ghcr.io/imazen/zen-metrics-sweep:0.6.4-ssim2-<sha>
#
# OR:
#
#   ssim2_backfill_chunk_worker.sh \
...
WORK_DIR="${WORK_DIR:-/tmp/ssim2-chunk-$$}"
...
OUT_SSIM2_R2=$(echo "$CHUNK_JSON" | jq -r '.out_sidecar_ssim2')

if [[ -z "$CHUNK_ID" || "$CHUNK_ID" == "null" ]]; then
    echo "ERROR: chunk_id missing from JSON" >&2
    exit 1
fi
if [[ -z "$OUT_SSIM2_R2" || "$OUT_SSIM2_R2" == "null" ]]; then
    echo "ERROR: out_sidecar_ssim2 missing from JSON" >&2
    exit 1
fi
...
echo "[ssim2-chunk-worker $CHUNK_ID] step 1/6: download input parquet" >&2
R2 cp "$INPUT_PARQUET_R2" "$WORK_DIR/$INPUT_PARQUET" >&2

echo "[ssim2-chunk-worker $CHUNK_ID] step 2/6: sync source basenames" >&2
...
echo "[ssim2-chunk-worker $CHUNK_ID] step 3/6: slice + group" >&2
...
echo "[ssim2-chunk-worker $CHUNK_ID] step 4/6: per-group sweep" >&2
...
while IFS='|' read -r gid codec q kj; do
    ...
    if (( sweep_rc != 0 )); then
        echo "[ssim2-chunk-worker $CHUNK_ID] step 4/6: sweep g${gid} FAILED rc=$sweep_rc codec=$codec q=$q" >&2
...
SIDECAR_SSIM2="$WORK_DIR/out/ssim2_imazen_v0_0_1.parquet"

echo "[ssim2-chunk-worker $CHUNK_ID] step 5/6: score-pairs ssim2-gpu" >&2
# ssim2-gpu has no small-image guard (unlike iwssim which requires >=176px).
# SSIMULACRA2 is the canonical v2 from Sneyers/Wang — no separate "extended"
# toggle exposed by zen-metrics; the default IS the canonical SSIMULACRA2 v2.
if [[ -n "$ZEN_METRICS_IMAGE" ]]; then
    docker run --rm $DOCKER_GPUS \
        --entrypoint /usr/local/bin/zen-metrics \
        -v "$WORK_DIR":"$WORK_DIR":rw \
        ...
        score-pairs \
            ...
            --out-parquet "$SIDECAR_SSIM2" \
            --gpu-runtime "$GPU_RUNTIME" 2>&1 | sed 's/^/  [ssim2] /' >&2
else
...
if [[ "$SKIP_UPLOAD" != "1" ]]; then
    echo "[ssim2-chunk-worker $CHUNK_ID] step 6/6: upload sidecar" >&2
    R2 cp "$SIDECAR_SSIM2" "$OUT_SSIM2_R2" >&2
else
    echo "[ssim2-chunk-worker $CHUNK_ID] step 6/6: SKIPPED upload" >&2
    echo "  ssim2 sidecar at: $SIDECAR_SSIM2" >&2
fi

echo "[ssim2-chunk-worker $CHUNK_ID] done" >&2
