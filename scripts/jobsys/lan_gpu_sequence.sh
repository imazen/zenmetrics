...
ssh -o BatchMode=yes -o ConnectTimeout=10 "$HOST" \
  "chmod +x lan_gpu_seq_run.sh; setsid nohup env ZM_KIND='$KIND' ZM_IMG='$IMG' ZM_S3BUCKET='$S3BUCKET' ZM_VRAM_CAP='${ZM_VRAM_CAP:-}' ZM_STORE='${ZEN_STORE:-tower}' ZM_PASS_TIMEOUT='${ZEN_PASS_TIMEOUT:-1800}' bash lan_gpu_seq_run.sh $* >/dev/null 2>&1 & echo \"\$(hostname): sequencer launched over: $* (store=${ZEN_STORE:-tower} vram_cap=${ZM_VRAM_CAP:-none})\""
