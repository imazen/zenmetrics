...
use zenmetrics_av1_compare::{Config, check_decode, encode};
#[derive(Deserialize)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    check_decode(&obu, req.config.width, req.config.height)?;
    ...
    println!(
        "{}",
        serde_json::json!({"protocol":"av1-api-i420-v1", "config":req.config,
