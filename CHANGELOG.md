...
- hdrvdp: **validated against official HDR-VDP-2.2.2** + `q` now
  reports official `res.Q` (**breaking semantic change**). Goldens
  were generated by running the official `hdrvdp-2.2.2` MATLAB
  release under Octave 11.1 on a deterministic 24-case luminance
  corpus (two sizes × `pixels_per_degree` {30, 60}); committed under
  `crates/hdrvdp/validation/` with the generator and
  `docs/VALIDATION.md` provenance. Worst-case deltas: `P_det`
  5.3e-14, `C_max` 3.9e-12 rel, `P_map` 3.9e-11, `res.Q` 9.9e-7;
  per-band `D` planes bit-agree to ~1e-16. The reference run
  required a real fix to upstream's own sources: `reconSpyr.m` /
  `reconSpyrLevs.m` detect the compiled `upConv` by comparing the
  last 3 chars of its path to `'.m'` — which can never match — so
  under Octave `is_mex` is wrongly true and the in-place `upConv`
  calls silently discard the hi-pass band and all 12 oriented bands
  from `S_map` (σ=4 noise: `C_max` 211.6 → 0.0002). The `q`/`q_mos`
  split is now upstream's exact convention: `q` = `res.Q = 100 −
  Σ(log(msre+ε)−log ε)·w_f` (was: a /N-normalized correlate
  ~−2→0); `q_mos` reproduces the logistic upstream *commented out*
  in 2.2.1 (`logistic(Q_raw)`, ≈4.4 on identical pairs — matches
  upstream's disabled output to 1e-5) and is documented as
  removed-as-unreliable. `tests/golden_2_2_2.rs` is the durable
...
### Added
- **Weighted rendezvous sharding — registered per-mode speed handicaps (user directive).** Epoch-sharded ownership now weighs workers by a committed registry (`fleet/handicaps.toml` — measured, cited, never auto-tuned): per-encoder-TYPE `encode` multipliers with a `default` fallback (per-encode concurrency re-ranks boxes per type, so cross-type extrapolation is deliberately refused), plus separate `metric` (CPU) and `gpu_metric` (the card) multipliers, selected per cell from the manifest (`JobKind` codec; the `_gpu` metric-name convention). Math: the canonical logarithm method `owner = argmax −w/ln(u)` over the frozen pair hash; the u-mapping is exact 52-bit IEEE arithmetic (two naive mappings provably hit `u = 1.0 → score −∞` at the top of the hash range — both boundary bugs caught by pin tests + an independent Python implementation before landing); `ln` is the pure-Rust `libm` so mixed x86/aarch64 fleets compute bit-identical scores; all-1.0/absent weights DELEGATE to the unweighted argmax — prior ownership bit-for-bit, pinned by test. Weight 0.0 = mode exclusion (role specialization; also blocks tail-steal for those cells). Precedence: campaign `RunControl.worker_weights` (wholesale override, next-pass fleet convergence; `RunControl` drops `Copy`/`Eq`) > binary-embedded committed registry > 1.0. Seeded from the avifgen attribution audit (`0f9fa073`): `encode.zenavif` = measured per-box distinct-done/h normalized to lianli 1.0 (i265 0.72, ryzen5800xt 0.67, tower 0.67, node-3 0.19, node-2 0.64 low-confidence, wsl-gate 0.57); svt/jxl/gainmap deliberately fall back to `default = 1.0` pending `scripts/jobsys/handicap_typebench.sh` (the committed per-type bench: registered manifest slices through the ordinary worker pass; smoke-verified end-to-end). Sim balance gate (900 cells, 3 workers at 20/15/10 cells/epoch): uniform sharding = 14-epoch shard-finish spread, 450 idle cell-slots, 30 epochs; correct weights = 1-epoch spread, 45 idle (−90%), 21 epochs — with total == distinct and zero post-bootstrap leases still holding. Tests: share proportionality (2.0 → 2×, ±8% rel), movement minimality (a raised weight only pulls to itself, ≈ the delta share), zero-weight exclusion, cross-computer determinism, mixed-codec per-type proportionality within one wave, exact-bit score pins, registry parse/sanity CI gate, worker-level role-exclusion end-to-end. (1eb1dbb1 = the core math; the registry/worker/bench/sim/docs land in its immediate successor commit)
- **Epoch-sharded claiming — a first-class claim mode (`lease` | `epoch-sharded`), opt-in, default unchanged.** Motivated by the avifgen encode campaign's measured 3.6× re-work (1,237,361 ledger rows for 343,465 distinct DONE cells) whose *dedup* was fixed by the ledger-snapshot contract (`92432e37`); this removes the claim *contention* itself. Workers re-shard the remaining gap at every wall-clock boundary (`epoch_len_secs`, default 600): pull the ledger snapshot (existing `--ledger-in` machinery, reused not duplicated), read the alive roster (per-epoch heartbeats under `claims/hb/{epoch}/`), partition by rendezvous hashing (spec-frozen FNV-1a 64 + splitmix64, pinned by vectors and an independent cross-implementation) — **zero claim traffic in steady state**, stable under roster churn (only a joined/departed worker's cells move). Leases remain the safety net: seam cells (ownership just moved) are claimed per-cell, un-rostered joiners run one lease-mode pass, and exhausted workers tail-steal peers' remaining cells lease-guarded after a heartbeating wait-to-tail (off with `--no-tail-steal`); exactly-once stays ledger-enforced. Config: worker `--claim-mode/--epoch-len-secs/--epoch-heartbeat-secs`, entrypoint `ZEN_CLAIM_MODE`/`ZEN_EPOCH_LEN_SECS`/`ZEN_EPOCH_HB_SECS`, and campaign-level `RunControl.claim_mode` (+ overrides) which converges a whole fleet centrally; one mode per run. Gates: core HRW stability/determinism/skew/seam tests; worker tests (2-worker zero-lease disjoint split, churn leases exactly the moved cells, tail-steal takeover under a virtual clock); `zenfleet-sim/tests/epoch_shard.rs` 3-worker fleet sim — total work exactly == distinct cells (the anti-3.6× assertion), killed-worker takeover at the boundary after heartbeats age out, steal duplicates bounded to stolen cells. Adoption: `docs/RUNNING_JOBS.md` §5b. (0636a38d, c9add527, ad1dd3a0)
...
- **Job-system overloaded ONE R2 bucket for read-only corpus + run-write** —
  ...
  **`zentrain`** (the training-pipeline bucket — all sweeps/backfills/features/
  picker-training/datasets live there), NOT `coefficient` (legacy vast.ai
  chunk-sweep ops) and NOT the throwaway `zen-tuning-ephemeral`; `ZEN_FLEET_BUCKET`
  default is now `zentrain`. Verified: neither minted cred can write codec-corpus
  (both `AccessDenied`). Rebuilt+pushed `zenfleet-worker:exec`. The GPU exec image
  (`:exec-gpu`) needs the same rebuild before its next GPU run (0de15e2b, f3a139be).
