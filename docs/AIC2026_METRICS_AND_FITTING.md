...
~~6. HDR-VDP-2 ingress config + HDR-VDP-3 → `HDR_VDP_2`/`HDR_VDP_3`
   columns.~~ **DONE (implementation) / unresolved (column)** — `hdrvdp3`
   (`crates/hdrvdp/src/v3`) is implemented and verified bit-faithful to
   **official MATLAB HDR-VDP-3.0.7** itself (Octave run on 3 pairs:
   9.696845/8.517431/9.645288 — identical to Rust and to the jpeg-ai-qaf
   `VDP3/` port @ `0628a6b`, mirrored at `~/tmp/jpeg-ai-qaf-hdrvdp3/VDP3/`).
   The *published column* is unreproduced: under the stated harness ingress
   (pq2lin codes + `L_refl=0.005·100/π`, clamp `L_max+L_refl`, rgb-bt.709,
   ppd≈64.06) both ours and the original MATLAB read ~0.36–0.92 JOD higher
   than published; ppd/task/EOTF/reflectance/swap/bsc probes all fail to
   close it → the column was generated by a different run config than the
   shipped code's defaults. `HDR_VDP_2` likewise misses our validated v2
   at both ppd 30 and 64.05. Recorded in `DIVERGENCES.md`.
